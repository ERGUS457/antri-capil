  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-6 py-10 text-gray-900">
      <div className="bg-white rounded-[20px] border border-gray-200 p-8 max-w-md w-full shadow-sm">
        ...
        <p className="text-sm text-gray-600 mt-1">Buat akun untuk booking & pantau status antrean.</p>
        ...
        <form onSubmit={submit} className="mt-6 flex flex-col gap-4">
          ...
          <label className="flex flex-col gap-1.5">
            <span className="text-sm font-semibold">NIK (16 digit, opsional tapi disarankan)</span>
          ...
          <button disabled={loading} type="submit" className="bg-blue-600 text-white rounded-full py-3.5 font-semibold hover:bg-blue-700 disabled:opacity-50">{loading ? 'Memproses...' : 'Daftar & Masuk →'}</button>
        </form>
        <p className="text-sm text-center text-gray-600 mt-6">Sudah punya akun? <Link href="/login" className="text-blue-600 font-semibold hover:underline">Masuk</Link></p>
        <p className="text-xs text-center text-gray-500 mt-3"><Link href="/" className="hover:underline">← Kembali ke Beranda</Link></p>
      </div>
    </div>
  );
}
