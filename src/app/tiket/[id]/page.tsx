...
type Tiket = {
  ...
  warga?: { nik: string; nama: string; email?: string | null } | null;
  user?: { nik: string | null; name: string; email: string } | null;
  ...
  return (
    <div className="min-h-screen bg-[#FFFBF0] py-10 px-6 print:bg-white">
      <div className="max-w-md mx-auto">
        ...
        <div className="bg-white rounded-[24px] border border-orange-100 overflow-hidden shadow-lg">
          ...
          <div className="p-6">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
              <div><p className="text-zinc-500">Nama</p><p className="font-semibold">{tiket.user?.name || tiket.warga?.nama || '-'}</p></div>
              <div><p className="text-zinc-500">NIK</p><p className="font-semibold">{tiket.user?.nik || tiket.warga?.nik || '-'}</p></div>
            ...
            <p className="mt-4 text-xs text-zinc-500 leading-relaxed">Tunjukkan QR ini di loket. Datang 15 menit sebelum jadwal. Tiket juga dikirim ke email {tiket.user?.email || tiket.warga?.email || '—'}.</p>
          </div>
        </div>
